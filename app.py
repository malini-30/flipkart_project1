...
# TODO: To restructure the program for efficient template and UI

# Useful libraries needed for the program implementation 
...
# Initializing the Flask app below
app = Flask(__name__)
app.config['SECRET_KEY'] = 'your-secret-key-change-in-production'
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///leave_management.db'
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False

# Using SQLAlchemy for ORM -  over raw SQL  is chosing for easier model management
#  to prevent SQL injection risks that our security  flagged last quarter
db = SQLAlchemy(app)
...
class User(UserMixin, db.Model):
    ...
    # Relationships between users below 
...
# Login manager user loader describes here
@login_manager.user_loader
def load_user(user_id):
    return User.query.get(int(user_id))

# Injecting the current datetime into templates 
...
# Dashboard.html is used below
# User Roles: Admin, Manager, Employee below
@app.route('/dashboard')
@login_required
def dashboard():
    #  from here Get dashboard statistics 
    stats = {}
    ...
    # Get recent leave requests from below
    if current_user.role == Role.ADMIN:
...
# Employees: Apply for leave, view status, edit or cancel requests from below
...
# Employee creating new leave request
...
def new_leave_request():
    if request.method == 'POST':
        try:
            ...
            days_requested = get_work_days(start_date, end_date)
            
            # Assigning to manager if employee has one
...
# View leave request details from below 
...
# Managers: Approve/reject leave, filter by status/employee/data
# Manager can reject or approve the leaves
@app.route('/leave-requests/<int:request_id>/approve', methods=['POST'])
@require_role([Role.MANAGER, Role.ADMIN])
def approve_leave_request(request_id):
    leave_request = LeaveRequest.query.get_or_404(request_id)
    
    # Manager can only approve their team's requests from below 
    if (current_user.role == Role.MANAGER and 
        leave_request.manager_id != current_user.id):
        flash('Access denied', 'error')
        return redirect(url_for('leave_requests'))
...
def reject_leave_request(request_id):
    ...
    #  here Manager can only reject their team's requests
    if (current_user.role == Role.MANAGER and 
        leave_request.manager_id != current_user.id):
        flash('Access denied', 'error')
        return redirect(url_for('leave_requests'))
...
def cancel_leave_request(request_id):
    ...
    # Only employee can cancel their own requests below
...
# Admin: view full dashboard with user management and  metrics
@app.route('/users')
...
# Reports for managers and admin
@app.route('/reports')
@require_role([Role.MANAGER, Role.ADMIN])
def reports():
...
def generate_report():
    ...
    """
    #  here Handle both form submission (POST) and quick reports (GET)
    if request.method == 'POST':
    ...
    else:  # GET request for quick reports below
    ...
    if report_type == 'monthly_summary':
        ...
        # This would need  here custom implementation based on your leave balance logic
        pass
    
    # Applying additional filters if provided
    if start_date:
    ...
    # Executing query and unpack results
    results = query.all()
    requests = [req for req, user in results]  # Extracting just the LeaveRequest objects
    
    if format_type == 'csv':
        return generate_report_csv(requests)
    elif format_type == 'pdf':
        return generate_pdf_leave(requests)

def generate_report_csv(requests):
    """Generate CSV report of leave requests"""
    output = io.StringIO()
    writer = csv.writer(output)
    
    # Header are written below  
    writer.writerow([
...
def generate_pdf_leave(leave_requests):
    """Generate company-branded PDF report"""
    buffer = io.BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter)
    styles = getSampleStyleSheet()
    
    # styles for company branding
    company_style = ParagraphStyle(
        name="CompanyStyle",
        fontSize=14,
        textColor=colors.HexColor("#2E86AB"),  #  giving Company color
...
# Error Handlers for 404 & 500 below
...
# Initializing the  database below
def create_tables():
    with app.app_context():
        db.create_all()
        
        # Create admin user if none exists below
        if not User.query.filter_by(role=Role.ADMIN).first():
            admin = User(
                username='admin',
                email='indu789@gmail.com',
                first_name='Busi Indu',
                last_name='User',
                role=Role.ADMIN,
        ...
        # Create default manager if none exists below
        if not User.query.filter_by(role=Role.MANAGER).first():
            manager = User(
                username='manager',
                email='hema456@gmail.com',
                first_name='Hema',
                last_name='Manager',
                role=Role.MANAGER,
            ...
            manager.set_password('mng123')
            db.session.add(manager)
            print("Created default manager user")

        # Create default employee if none exists below
